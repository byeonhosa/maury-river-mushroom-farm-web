...
The master mushroom species catalog is the owner-approved 18-species lineup (Decisions D15/D16): `BO` Blue Oyster, `LM` Lion's Mane, `STK` Shiitake, `CNT` Chestnut, `PO` Pink Oyster, `WO` White Oyster, `KT` King Trumpet, `BKO` Black King Oyster, `PP` Pioppino, `MTK` Maitake, `BCH` Beech, `NMK` Nameko, `GEN` Golden Enoki, `WEN` White Enoki, `CDY` Cordyceps, `GO` Golden Oyster (wholesale-only), `RSH` Reishi, and `TT` Turkey Tail. Each species carries an `availabilityTier` (`year-round`, `in-rotation-now`, `returning`, `wholesale-only`, `functional-coming-later`). King Blue and Elm Oyster were retired from the lineup. Species can be educational/catalog entries even when no customer-facing product is currently available.
