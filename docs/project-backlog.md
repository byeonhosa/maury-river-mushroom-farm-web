...
- Confirm final pickup locations, pickup windows, preorder cutoffs, local delivery rules, and market schedules.
- Confirm final prices, unit sizes, inventory statuses, and launch availability for every product.
- Move storefront from hybrid Medusa/fallback reads to required Medusa reads once local Store API keys, regions, and production checkout settings are finalized.
- Configure Medusa shipping options, service zones, pickup/local-delivery fulfillment, publishable API keys, and checkout rules after final pickup/shipping policy decisions.
